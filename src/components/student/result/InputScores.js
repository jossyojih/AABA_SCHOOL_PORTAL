...
function InputScores({ score, scores, setScores, i, data }) {
  ...
  useEffect(() => {
    ...
    console.log(data.section, "The sectionnnnn!")
    // Senior secondary scale is different!
    if (data.section === "Senior-Secondary") {
      if (total > 79.9) {
        setGrade('A')
        setRemark('Excellent')
      } else if (total > 69.9 && total < 80) {
        setGrade('B')
        setRemark('Very Good')

      } else if (total > 59.9 && total < 70) {
        setGrade('C')
        setRemark('Good')
