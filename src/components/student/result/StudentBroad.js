    ...
    <div className='classBroadSheet'>
        ...
        <table className='BroadSheet'>
            <thead >
                <tr >
                    <th>Names</th>
                    {result[0]?.scores.map((sub,i)=><th key={i} colSpan="8">{sub.subject}</th>)}
                    <th colSpan='3'>Marks Obtained</th>
                </tr>
                <tr>
                  
                  {result[0]?.scores.map((sub,i)=><><td></td><td >test</td><td >exam</td><td>total</td><td>Grade</td><td>Remark</td><td>Subject position</td><td>Class Average</td></>)}
                ...
                {result.map((score,i)=>{
                    return (
                        <tr key={i}>
                            ...
                            {result[i].scores.map((sub,i)=><><td >{sub.CA.total}</td><td >{sub.exam}</td><td>{sub.total}</td><td>{sub.grade}</td><td>{sub.remark}</td><td>{sub.subjectPosition}</td><td>{sub.classAverage}</td><td></td></>)}
